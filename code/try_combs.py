...
MIN_NTW = 20                       # minimal number of windows per combo
# step policy per window size to avoid tiny n_tw and poor convergence
STEPS_POLICY = {
    30: [1],
    40: [1,15],
    60: [1,15,30],
...
# reset tracking files on each run
...
method_param_ranges = {
    "ae":{"layer1":[512],"layer2":[256],"layer3":[32,64,128]},
...
def steps_for_tw(tw_len):
    # apply policy and ensure enough windows (>= MIN_NTW)
    allowed = STEPS_POLICY.get(tw_len, steps)
    allowed = [s for s in allowed if s in steps]
    return [s for s in allowed if compute_n_time_windows(duration_sec, TR, tw_len, s) >= MIN_NTW]
...
for method, params in method_param_combinations: log(f"  {method}: {params}")

# === FULL GRID (after policy) ===
full_combinations = []
for n_nodes in n_nodes_options:
    for method, method_params in method_param_combinations:
        for k in k_list:
            for tw_len in time_window_lengths:
                for step in steps_for_tw(tw_len):
                    full_combinations.append((n_nodes, method, method_params, k, tw_len, step))

total = len(full_combinations)
log(f"Total combinations to run (after policy): {total}")
...
with open(combinations_file, 'w') as f: json.dump(comb_data, f, indent=2)
...
for idx, (n_nodes, method, method_params, k, tw_len, step) in enumerate(full_combinations, start=1):
    ...
    # additional guard: skip if window size too small vs nodes
    threshold = n_nodes * MIN_TW_RATIO
    if tw_len < threshold:
        log(f"Skip {cid} (tw_len {tw_len} < n_nodes/10 {threshold:.1f})")
        progress[cid] = {"status":"skipped","reason":"tw_len < n_nodes/10",
                         "params":params_dict,"skipped_at":datetime.datetime.now().isoformat(),
                         "threshold":threshold}
